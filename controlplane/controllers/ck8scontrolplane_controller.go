...
func (r *CK8sControlPlaneReconciler) updateStatus(ctx context.Context, kcp *controlplanev1.CK8sControlPlane, cluster *clusterv1.Cluster) error {
	...
	// When default network is disabled, nodes remain NotReady until the external CNI pack is
	// installed by Palette's reconcileLayer. Mark the control plane Ready/Available as soon as
	// it is initialized so that ControlPlaneInitializedCondition propagates to the Cluster object
	// and the CAPI ClusterCacheTracker can establish a remote connection (IsAPIServerOnline).
